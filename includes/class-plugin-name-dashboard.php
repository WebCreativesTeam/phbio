...
<div x-data="dashboard" 
    ...
    <div x-show="(activeTab === 'profile' || activeTab === 'links') && !showTemplates && !showSettings" class="pb-6 -mt-12 tab-content md:pt-5 md:px-2">
    ...
    <div x-show="editMode && !showTemplates && !showSettings" class="content-preview">
...
<?php include_once plugin_dir_path(__FILE__) . 'class-plugin-name-footer.php'; ?>
        ...
        ?>
            ...
            <div class="mt-10 w-[89%] sm:w-[94%] mx-auto">
                ...
                <?php 
                    ...
                    Plugin_Name_Builder::checkbox_field('phbio_logo', 
                    ...
                    if(!Plugin_Name_Utilities::check_user_capability(Plugin_Name_Capabilities::MANAGE_WEBSITE_LOGO)) {
                        echo '<div class="warning-message"><svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 warning-icon" viewBox="0 0 576 512" fill="#f1441e"><path d="M316.9 18C311.6 7 300.4 0 288.1 0s-23.4 7-28.8 18L195 150.3 51.4 171.5c-12 1.8-22 10.2-25.7 21.7s-.7 24.2 7.9 32.7L137.8 329 113.2 474.7c-2 12 3 24.2 12.9 31.3s23 8 33.8 2.3l128.3-68.5 128.3 68.5c10.8 5.7 23.9 4.9 33.8-2.3s14.9-19.3 12.9-31.3L438.5 329 542.7 225.9c8.6-8.5 11.7-21.2 7.9-32.7s-13.7-19.9-25.7-21.7L381.2 150.3 316.9 18z"/> </svg><span>' . self::get_error_logo($user_id) . '</span></div>';
                    }  
                 ?>
                </form>
    ...
    <div class="max-w-[700px] mt-40 pb-20 mx-auto">
        ...
        <?php
            ...
            <?php } else {  ?>
                <div class="range-filters">
                <span @click="setDateRange('lifetime', true)" class="range-filter">Lifetime</span>
                <span @click="setDateRange('90days', true)" class="range-filter">90 D</span>
                <span @click="setDateRange('30days', true)" class="range-filter">30 D</span>
                <span @click="setDateRange('7days', true)" class="range-filter">7 D</span>
                <span @click="setDateRange('Today', true)" class="range-filter">Today</span>
                <span @click.stop="setDateRange('custom')" class="range-filter">Custom</span>
            </div>
            <?php } ?>
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                if (isset($_POST['date_from']) && isset($_POST['date_to'])) {
                    ...
                    echo "<div class='table-wrapper'>";     
                        echo do_shortcode('[wpdatatable id=17 var1=' . $date_from . ' var2=' . $date_to . ']');
                    echo "</div>";
                    echo Plugin_Name_Utilities::is_empty_table(17);
                    ...
                    echo "<div class='table-wrapper'>";     
                    echo do_shortcode('[wpdatatable id=19 var2=' . $date_from . ' var3=' . $date_to . ']');
                    echo "</div>";
                    ...
                    echo "<div class='chart-wraper'>";        
                    echo do_shortcode('[wpdatachart id=8]');
                    echo "</div>";
                    ...
                    echo "<div class='table-wrapper'>";     
                        echo do_shortcode('[wpdatatable id=24 var2=' . $date_from . ' var3=' . $date_to . ']');
                    echo "</div>";
    ...
    public function area__preview($user_id) {
        ...
        echo '<div class="iframe-container">
                <div class="loaad">
                    ...
                    <div class="text-base text-[#171717] mt-1 text-center px-4 md:px-0">
...
<?php if (!Plugin_Name_Utilities::is_full_version($user_id)) {
    ...
    <?php echo '<div class="mt-6 warning-message"><svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 warning-icon" viewBox="0 0 576 512" fill="currentColor"><path d="M316.9 18C311.6 7 300.4 0 288.1 0s-23.4 7-28.8 18L195 150.3 51.4 171.5c-12 1.8-22 10.2-25.7 21.7s-.7 24.2 7.9 32.7L137.8 329 113.2 474.7c-2 12 3 24.2 12.9 31.3s23 8 33.8 2.3l128.3-68.5 128.3 68.5c10.8 5.7 23.9 4.9 33.8-2.3s14.9-19.3 12.9-31.3L438.5 329 542.7 225.9c8.6-8.5 11.7-21.2 7.9-32.7s-13.7-19.9-25.7-21.7L381.2 150.3 316.9 18z"></path></svg><span>' . self::get_error_template($user_id) . '</span></div>'; ?>
    </div>
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                if (isset($_POST['date_from']) && isset($_POST['date_to'])) {
                    ...
                    if(Plugin_Name_Utilities::is_full_version($user_id)) {
                        ...
                        echo "<div class='analytics-label input-label'><svg xmlns='http://www.w3.org/2000/svg' height='1em' viewBox='0 0 512 512' class='heading-icon' fill='currentColor'><path d='M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zm0-352a96 96 0 1 1 0 192 96 96 0 1 1 0-192z'></path></svg>  Performance</div>";
                        echo "<div><a class='py-2 unlock-more' href='/upgrade' target='_blank'><svg xmlns='http://www.w3.org/2000/svg' height='1em' viewBox='0 0 448 512' fill='currentColor'><path d='M144 144v48H304V144c0-44.2-35.8-80-80-80s-80 35.8-80 80zM80 192V144C80 64.5 144.5 0 224 0s144 64.5 144 144v48h16c35.3 0 64 28.7 64 64V448c0 35.3-28.7 64-64 64H64c-35.3 0-64-28.7-64-64V256c0-35.3 28.7-64 64-64H80z'></path></svg>" . Plugin_Name_Utilities::is_user_suspended_analytics_error_text($user_id) . "</a></div>";
                        echo "</div>";
                    }

                    if(Plugin_Name_Utilities::is_full_version($user_id)) {   
                        echo "<div class='chart-wraper'>";     
                        echo do_shortcode('[wpdatachart id=10]');
                        echo "</div>";
                        echo Plugin_Name_Utilities::is_empty_chart('area', 22, 10);
                    } else {
                        echo "<div class='chart-wraper'>";     
                    ...
                    if(Plugin_Name_Utilities::is_full_version($user_id)) {
                        ...
                        echo "</div>";
                        echo Plugin_Name_Utilities::is_empty_chart('pie', 7, 4);
                ...
                echo "<div class='table-wrapper'>";     
                echo do_shortcode('[wpdatatable id=22 var1=' . $post->ID . ' var2=' . date("Y-m-d") . ' var3=' . date("Y-m-d") . ']');
                echo "</div>";
                echo Plugin_Name_Utilities::is_empty_table(22);
                ...
                if(Plugin_Name_Utilities::is_full_version($user_id)) {
                    ...
                    $social_img = plugin_dir_url( __FILE__ ) . 'img/social.jpg'; ?>
                    <div class="bg-cover bg-center w-[250px] h-[250px] md:w-[400px] md:h-[400px] max-w-full" style="background-image: url('<?php echo $social_img; ?>');">
                            <div class="text-2xl md:text-4xl flex justify-center items-center w-[250px] h-[250px] md:w-[400px] md:h-[400px] bg-white bg-opacity-80"> Sample Data</div>
                    </div>
