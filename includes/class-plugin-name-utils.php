...
class Plugin_Name_Utilities {
    ...
    public static function current_user_has_backup_links() {
        ...
        if(self::user_has_backup($user_id) && self::is_lite_version($user_id)) {
            ...
            <span>Your
            subscription has expired, but don\'t worry! We\'ll keep all your links live for <b>' . self::get_remaining_hours_for_user_backup($user_id) . '</b>. <a href="/my-account/subscription" class="font-semibold no-underline text-[#F1441E]" target="_blank" rel="nofollow">Renew now</a> to keep
